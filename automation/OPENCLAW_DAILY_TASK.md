...
当前合同版本为 `3.3.0`。如果现有任务正文没有 `[ALUX_DAILY_CONTRACT_VERSION: 3.3.0]`，保留原任务 ID、执行时间、时区和凭据，只替换任务正文，完成一次手动 dry run 后再等待下一次定时执行；不要新建一个重复的定时任务。

## 应写入自动化的任务正文

```text
[ALUX_DAILY_CONTRACT_VERSION: 3.3.0]
...
6. 以官方与一手来源为主，面向大众追踪全球 AI 新功能、AI Agent、GitHub 开源、Skills、软硬件、系统架构和市场变化。大厂与小项目按有趣、有用、有意义的程度选择，不强制每条映射 ALUX。主文通常 7-9 条且至少覆盖 4 类；产品/模型/软件更新合计不超过 50%，至少 2 条开源或 Skill，并至少 1 条硬件、架构、研究或有解释力的市场信号。
