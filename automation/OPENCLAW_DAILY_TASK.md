...
当前合同版本为 `3.5.0`。如果现有任务正文没有 `[ALUX_DAILY_CONTRACT_VERSION: 3.5.0]`，保留原任务 ID、执行时间、时区和凭据，只替换任务正文，完成一次手动 dry run 后再等待下一次定时执行；不要新建一个重复的定时任务。

## 应写入自动化的任务正文

```text
[ALUX_DAILY_CONTRACT_VERSION: 3.5.0]
...
5. 页面布局、类名、颜色、字体层级和响应式行为只以 `templates/report-master.json` 固定的 `2026-08-21-editorial-v2` 为母版；复制该母版结构和 CSS，只替换当日日期、事实、标题、数字、链接、摘要和判断。不得改用最近一期滚动继承，也不得复制旧事实和旧判断。
6. 以官方与一手来源为主，面向大众追踪全球 AI 新功能、AI Agent、GitHub 开源、Skills、软硬件、系统架构和市场变化。生成候选池前，必须先运行私有的大厂开源扫描，覆盖官方 GitHub 与 Hugging Face，回看 14 天，并对每条 `must-review` 留下选入、已报道或不构成实质开放发布的证据。已确认的重要大厂模型权重、代码、数据集或框架发布必须优先入选，不得被普通版本更新挤掉。每天核对持续观察项；预告阶段不重复报道，官方开放状态变化时立即入选。所有 GitHub 开源/Skill 推荐在成稿当天刷新 Stars 和许可证状态；Stars 只表示关注度快照，不参与质量排名。每条信息保留准确发布日期与当天观察日期，超过 14 天只有出现可核验的新变化才可入选。大厂与小项目在进入候选后仍按有趣、有用、有意义的程度编辑。主文通常 7-9 条且至少覆盖 4 类；产品/模型/软件更新合计不超过 50%，至少 2 条开源或 Skill，并至少 1 条硬件、架构、研究或有解释力的市场信号。必须区分 OSI 开源、开源代码、开放权重和自定义社区许可。
