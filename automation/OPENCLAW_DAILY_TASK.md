...
当前合同版本为 `3.4.0`。如果现有任务正文没有 `[ALUX_DAILY_CONTRACT_VERSION: 3.4.0]`，保留原任务 ID、执行时间、时区和凭据，只替换任务正文，完成一次手动 dry run 后再等待下一次定时执行；不要新建一个重复的定时任务。

## 应写入自动化的任务正文

```text
[ALUX_DAILY_CONTRACT_VERSION: 3.4.0]
