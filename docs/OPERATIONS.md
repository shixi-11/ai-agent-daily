...
`templates/report-master.json` 固定日报的正式视觉与编辑母版。当前母版是 `2026-08-26-editorial-v3.1`，指向 `content/zh/20260826_ALUX_AI智能体情报日报.html`；新一期只复制它的结构与 CSS，并遵守 6-9 条（通常 7-9 条，只有没有第七条通过时效、来源和读者价值门禁时才允许 6 条）、发布类不超过 50%、首屏精简、统计语义分离和导读/雷达分工，不以最近一期滚动继承版式。
