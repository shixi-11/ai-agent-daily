...
当前自动化合同版本为 `3.6.0`。OpenClaw 发现现有日报任务没有 `[ALUX_DAILY_CONTRACT_VERSION: 3.6.0]` 时，必须保留原任务 ID、执行时间、时区和凭据，只替换任务正文并完成一次 dry run；不得另建一个重复任务。日报主任务使用 `openai/gpt-5.6-sol`、`thinking=high`、标准速度并禁用 fallback；验收与恢复继续按各自无模型 command payload 执行，只有内容级恢复才唤起同一模型。
...
5. 新一期必须读取 `templates/report-master.json`，以其中固定的 `2026-08-26-editorial-v3` 为唯一日报母版；复制结构和 CSS，只替换当日内容。主文 7-9 条，产品/模型/软件更新不超过 50%；Hero 四项统计必须分别表达值得关注、可动手试、开源发现和覆盖区域。`今日AI导航` 只做事实地图，`AI Agent雷达` 只做跨信号综合判断。新闻正文在上，“试试看 / 注意点”在下；公开页不出现选题边界、观察理由或其他编辑过程话术。
   每天另行核对 OpenAI Codex 自动/全局额度重置和重置卡活动；只有官方确认的公开事件进入正文，精确时刻统一换算为北京时间，官方没给时刻则明确写未公布，不能猜测。
