...
当前自动化合同版本为 `3.5.1`。OpenClaw 发现现有日报任务没有 `[ALUX_DAILY_CONTRACT_VERSION: 3.5.1]` 时，必须保留原任务 ID、执行时间、时区和凭据，只替换任务正文并完成一次 dry run；不得另建一个重复任务。日报主任务、验收任务和恢复任务统一使用 `openai/gpt-5.6-sol`、`thinking=high`、标准速度并禁用 fallback。
